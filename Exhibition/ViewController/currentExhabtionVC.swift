...
class currentExhabtionVC: UIViewController {
    
    var singleItem: currentFaire?
...
//    @IBOutlet weak var holeNumberFourTableView: UITableView!
    ...
    @IBOutlet weak var mapIllstraitonCollectionView: UICollectionView!
...
//        holeNumberOneTableView.dataSource = self
        ...
        mapIllstraitonCollectionView.delegate = self
        mapIllstraitonCollectionView.dataSource = self
        setupNavagtion()
        textEnabeld()
...
//
        mapIllstraitonCollectionView.layer.cornerRadius = 5.0
        mapIllstraitonCollectionView.layer.borderWidth = 2.0
        mapIllstraitonCollectionView.layer.borderColor = #colorLiteral(red: 0.9333333333, green: 0.9333333333, blue: 0.9333333333, alpha: 1)
    }
    ...
    func loadYoutube(videoID:String) {
        ...
        guard
            ...
            let youtubeURL = URL(string: "https://www.youtube.com/embed/\(newUrl ?? "")")
            else { return }
    ...
    @objc private func handleRefreshMapIllestration() {
        ...
        API_home.imageMaping(id_fair: singleItem?.id ?? 0){ (error: Error?, mapCollection: [imgMapIllustration]?) in
            if let mapCollection = mapCollection {
                ...
                self.mapIllstraitonCollectionView.reloadData()
            }
        }
    ...
    @objc private func handleRefreshHolasOne() {
        API_CurrentFairs.holeExhibitorNos(url: URLs.hole_exhibitor_No_1,id_fair: singleItem?.id ?? 0){ (error: Error?, holes: [holeExhibitorNo]?) in
            if let holes = holes {
                ...
                self.textEnabeld()
                //self.holeNumberOneTableView.reloadData()
            }
        }
    ...
    @objc private func handleRefreshHolasTow() {
        API_CurrentFairs.holeExhibitorNos2(url: URLs.hole_exhibitor_No_2,id_fair: singleItem?.id ?? 0){ (error: Error?, holes2: [holeExhibitorNo2]?) in
            if let holes2 = holes2 {
                ...
                self.textEnabeld()
                //self.holeNumberTwoTableView.reloadData()
            }
        }
    ...
    @objc private func handleRefreshHolas3() {
        API_CurrentFairs.holeExhibitorNos3(url: URLs.hole_exhibitor_No_3,id_fair: singleItem?.id ?? 0){ (error: Error?, holes3: [holeExhibitorNo3]?) in
            if let holes3 = holes3 {
                ...
                self.textEnabeld()
               // self.holeNumber3TableView.reloadData()
            }
        }
    ...
    @objc private func handleRefreshHolas4() {
        API_CurrentFairs.holeExhibitorNos4(url: URLs.hole_exhibitor_No_4,id_fair: singleItem?.id ?? 0){ (error: Error?, holes4: [holeExhibitorNo4]?) in
            if let holes4 = holes4 {
                ...
                self.textEnabeld()
               // self.holeNumberFourTableView.reloadData()
            }
        }
...
extension currentExhabtionVC: UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let cell = suponcerCollection.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? curentFarieDetialsCell {
            ...
            if let cell = mapIllstraitonCollectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? mapIllustrationCell {
...
//extension currentExhabtionVC: UITableViewDelegate,UITableViewDataSource {
//
...
extension currentExhabtionVC: UIPickerViewDelegate, UIPickerViewDataSource {
